...
public class Move2D extends Entity2DComponent{
	...
	public void input(float delta){
		float moveAmt = 200*delta;
		
		if(Input.getKey(Input.KEY_UP) && !Input.getKey(Input.KEY_DOWN)){
			getTransform().setPos(new Vector2f(getTransform().getPos().getX(), getTransform().getPos().getY()+moveAmt));
		}
		if(Input.getKey(Input.KEY_DOWN) && !Input.getKey(Input.KEY_UP)){
			getTransform().setPos(new Vector2f(getTransform().getPos().getX(), getTransform().getPos().getY()-moveAmt));
		}
		if(Input.getKey(Input.KEY_RIGHT) && !Input.getKey(Input.KEY_LEFT)){
			getTransform().setPos(new Vector2f(getTransform().getPos().getX()+moveAmt, getTransform().getPos().getY()));
		}
		if(Input.getKey(Input.KEY_LEFT) && !Input.getKey(Input.KEY_RIGHT)){
			getTransform().setPos(new Vector2f(getTransform().getPos().getX()-moveAmt, getTransform().getPos().getY()));
		}
	}
}
