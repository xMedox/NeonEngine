...
//	private float modelScale = 2;
	private float modelScale = 4;
	
	private boolean aim = false;
	private boolean aimed = false;
	
//	private Vector3f noPos = new Vector3f(0.325f*modelScale, /*-0.4f*modelScale*/-0.95f/2*modelScale, 1.0f*modelScale);
	private Vector3f noPos = new Vector3f(0.35f*modelScale, /*-0.4f*modelScale*/0f/2*modelScale, 0.6f*modelScale);
...
//	private Vector3f noPos = new Vector3f(0.45f*modelScale, -0.4f*modelScale, 1.2f*modelScale);
	private Vector3f yesPos = new Vector3f(0*modelScale, /*-0.28f*modelScale*/0f/2*modelScale, 0.5f*modelScale);
...
//	private float movingTime = 70;
	...
	public WeaponScript(Player player){
		this(player, new InputKey(Input.MOUSE, Input.BUTTON_RIGHT));
	}
	
	public WeaponScript(Player player, InputKey aimKey){
		this.player = player;
...
//					zoom = noZoom;
		...
		if(aimed || aim){
			player.getSprintMove().setEnableSprint(false);
		}else{
			player.getSprintMove().setEnableSprint(true);
		}
